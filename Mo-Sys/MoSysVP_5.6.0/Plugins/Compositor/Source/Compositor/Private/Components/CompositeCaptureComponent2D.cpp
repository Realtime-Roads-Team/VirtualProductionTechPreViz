...
void UCompositeCaptureComponent2D::ViewportResizeCallback(FViewport* Viewport, uint32 Unused)
{
	...
	if (GetIsCompositeCaptureActive())
	{
		if (ViewportSize.X > 2 && ViewportSize.Y > 2)
		{
			...
			FIntPoint NewTargetTextureSize = FIntPoint(FMath::Max(static_cast<float>(ViewportSize.X) * NormalizedScreenPercentage, 2.F), FMath::Max(static_cast<float>(ViewportSize.Y) * NormalizedScreenPercentage, 2.F));
#if WITH_EDITOR
			// James added to force reflection target to 16:9 when in PIE
			if (GEditor && (GEditor->PlayWorld || GIsPlayInEditorWorld))
			{
				// Scaled HD
				NewTargetTextureSize = FIntPoint(1920.f * NormalizedScreenPercentage, 1080.f * NormalizedScreenPercentage);
			}
#endif
...
void UCompositeCaptureComponent2D::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (CompositorSubsystem)
	{
		...
		if (!CompositeViewport->ViewportResizedEvent.IsBoundToObject(this))
		{
			TryAddCompositeViewportResizeEvent();
		}		
...
void UCompositeCaptureComponent2D::UpdateSceneCaptureContents(FSceneInterface* Scene, ISceneRenderBuilder& SceneRenderBuilder)
{
...
bool UCompositeCaptureComponent2D::TryAddCompositeViewportResizeEvent()
{
	if (CompositorSubsystem)
	{
		FViewport* CompositeViewport = CompositorSubsystem->GetCompositeViewport();
		...
		{
			CompositeViewport->ViewportResizedEvent.RemoveAll(this);
			CompositeViewport->ViewportResizedEvent.AddUObject(this, &UCompositeCaptureComponent2D::ViewportResizeCallback);

			// Call it once to make sure the target textures are up to date.
			ViewportResizeCallback(CompositeViewport, 0);
			return true;
		}
	}	
