...
{	
	// Mo-Sys changed
	FPlacementCategoryInfo Info( LOCTEXT("PlacementMode_Compositor", "Compositor"),
		FSlateIcon(FCompositorEditorStyle::GetStyleSetName(), TEXT("CompositorEditor.OpenPluginWindow")), "Compositor", TEXT("PMCompositor"), 47 );

	IPlacementModeModule::Get().RegisterPlacementCategory(Info);

	IPlacementModeModule::Get().RegisterPlaceableItem(Info.UniqueHandle, MakeShareable( new FPlaceableItem(nullptr, FAssetData(ACompositeMesh::StaticClass())) ));
	IPlacementModeModule::Get().RegisterPlaceableItem(Info.UniqueHandle, MakeShareable(new FPlaceableItem(nullptr, FAssetData(ACompositePlanarReflection::StaticClass()))));
}
